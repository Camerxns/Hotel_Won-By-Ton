                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <table class="table table-striped mx-auto">
                        <thead>
                            <tr>
                                <th class="px-6">ID</th>
                                <th class="px-6">Room ID</th>
                                <th class="px-6">Check In Date</th>
                                <th class="px-6">Check Out Date</th>
                                <th class="px-6">Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($reservationList as $index => $data)
                                <tr>
                                    <td class="px-6">{{ $index + 1 }}</td>
                                    <td class="px-6">{{ $data->RoomID }}</td>
                                    <td class="px-6">{{ $data->CheckinDate }}</td>
                                    <td class="px-6">{{ $data->CheckoutDate }}</td>
                                    <td class="px-6">
                                        <div style="display: flex">                                    
                                            <form action="{{ route('reservations.destroy', $data->ReservationID) }}" method="post">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-danger">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="17.64" height="17.64" fill="currentColor" class="bi bi-x" viewBox="0 0 16 16">
